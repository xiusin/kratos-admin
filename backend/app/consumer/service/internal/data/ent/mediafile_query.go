...
//		GroupBy(mediafile.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.MediaFile.Query().
//		Select(mediafile.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (_q *MediaFileQuery) prepareQuery(ctx context.Context) error {
	...
	if mediafile.Policy == nil {
		return errors.New("ent: uninitialized mediafile.Policy (forgotten import ent/runtime?)")
	}
	if err := mediafile.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *MediaFileQuery) ForUpdate(opts ...sql.LockOption) *MediaFileQuery {
...
func (_q *MediaFileQuery) ForShare(opts ...sql.LockOption) *MediaFileQuery {
	if _q.driver.Dialect() == dialect.Postgres {
		_q.Unique(false)
	}
	_q.modifiers = append(_q.modifiers, func(s *sql.Selector) {
