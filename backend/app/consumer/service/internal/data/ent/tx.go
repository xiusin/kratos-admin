...
func (tx *Tx) init() {
	...
	tx.FreightTemplate = NewFreightTemplateClient(tx.config)
	tx.LoginLog = NewLoginLogClient(tx.config)
	tx.LogisticsTracking = NewLogisticsTrackingClient(tx.config)
	...
	tx.TenantConfigHistory = NewTenantConfigHistoryClient(tx.config)
}
...
// applies a query, for example: Consumer.QueryXXX(), the query will be executed
